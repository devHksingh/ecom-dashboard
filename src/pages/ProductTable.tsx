...
const ProductTable = () => {
  ...
  useEffect(() => {
    if (data?.products) {
      ...
      console.log(
        "lastThirtyDaysProductCount",
        data.lastThirtyDaysProductCount
      );
  ...
  if (isError) {
    ...
    const errorMessage =
      ...
      "Something went wrong!.Error loading products. Please try again later.Or refresh the page";
      ...
      <div className="p-6 w-[90%] bg-card/75 text-copy-primary md:w-[96%] mx-auto rounded-md">
        ...
        <div className="grid gap-4 mt-2 mb-4 auto-rows-min md:grid-cols-3">
          <div className="flex flex-col gap-2 p-2 rounded-xl bg-stone-400/50 aspect-auto">
            <h2 className="flex justify-between w-full font-semibold capitalize text-md">
              total Products:{" "}
              <span>
                {" "}
                <Warehouse className="text-sky-500" />
              </span>
            </h2>
            <span className="font-mono text-4xl font-bold">
              {totalproducts}
            </span>
            <span className="text-sm font-medium text-copy-primary/70 text-pretty">
              +{lastMountAddedProduct} from last month
            </span>
          </div>
          <div className="flex flex-col gap-2 p-2 rounded-xl bg-stone-400/50 aspect-auto">
            <h2 className="flex justify-between w-full font-semibold capitalize text-md">
          ...
          <div className="flex flex-col gap-2 p-2 rounded-xl bg-stone-400/50 aspect-auto">
            <h2 className="flex justify-between w-full font-semibold capitalize text-md">
              total stock Quantaty:{" "}
              <span>
                <Store className="text-sky-500" />
              </span>
            </h2>
            <span className="font-mono text-4xl font-bold">
              {data.productStockStatus.stockQuantaty}
            </span>
          </div>
          <div className="flex flex-col gap-2 p-2 rounded-xl bg-stone-400/50 aspect-auto">
            <h2 className="flex justify-between w-full font-semibold capitalize text-md">
            ...
            <span className="font-mono text-4xl font-bold">
              {data.productStockStatus.inStock}
            </span>
          </div>
          <div className="flex flex-col gap-2 p-2 rounded-xl bg-stone-400/50 aspect-auto">
            <h2 className="flex justify-between w-full font-semibold capitalize text-md">
            ...
            <span className="font-mono text-4xl font-bold">
              {data.productStockStatus.lowStock}
            </span>
          </div>
          <div className="flex flex-col gap-2 p-2 rounded-xl bg-stone-400/50 aspect-auto">
            <h2 className="flex justify-between w-full font-semibold capitalize text-md">
            ...
            <span className="font-mono text-4xl font-bold ">
              {data.productStockStatus.outOfStock}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <Link
            to={"/dashboard/product/createProduct"}
            className="self-center px-2 py-1 text-white bg-indigo-500 rounded-lg hover:bg-indigo-600"
          >
            Add Product
          </Link>
        </div>
                    ...
                    >
                      ...
                      >
                        ...
                        {header.column.getIsSorted() === "asc" && (
                          <ArrowUp01Icon className="ml-2" size={14} />
                        )}
                        {header.column.getIsSorted() === "desc" && (
                          <ArrowBigDownIcon className="ml-2" size={16} />
                        )}
                      </div>
                    </th>
                  ))}
                ...
                table.getRowModel().rows.map((row) => (
                  <tr key={row.id} className="hover:bg-gray-50">
                    {row.getVisibleCells().map((cell) => (
                      <td
                        key={cell.id}
                        className="px-6 py-4 text-sm font-medium text-gray-500 whitespace-nowrap"
                      >
                        {flexRender(
